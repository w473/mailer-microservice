...
exports.getAllTemplates = (req, res, next) => {
  const where = {};
  return Template.findAndCountAll({
    where: where,
    limit: req.query.limit ?? 10,
    offset: req.query.offset ?? 0,
    include: [{ model: TemplateLocale }]
  })
    .then((result) => {
      if (result.count > 0) {
        return res.status(200).json({
          templates: TemplateFormatter.formatAll(result.rows),
          total: result.count
        });
      }
...
exports.getTemplate = (req, res, next) => {
  return Template.findByPk(req.params.templateId, {
    include: [{ model: TemplateLocale }]
  })
    .then((template) => {
      if (template) {
        return res.status(200).json(TemplateFormatter.formatOne(template));
      }
  ...
  return Template.findByPk(req.params.templateId)
    .then((template) => {
      if (template) {
        ...
        return template.save().then(() => res.status(204).send());
      }
      return res.status(404).json({ message: 'Template has not been found' });
    })
    .catch((err) => next(err));
...
exports.deleteTemplate = (req, res, next) => {
  return Template.destroy({ where: { id: req.params.templateId } })
    .then((deletedCount) => {
      if (deletedCount > 0) {
...
exports.updateTemplateLocale = (req, res, next) => {
  return TemplateLocale.findOne({
    ...
    .then((template) => {
      ...
      return res
        .status(404)
        .json({ message: 'Template Locale has not been found' });
    })
    .catch((err) => next(err));
};

exports.deleteTemplateLocale = (req, res, next) => {
  return TemplateLocale.destroy({
    where: {
      templateId: req.params.templateId,
      locale: req.params.locale
    }
